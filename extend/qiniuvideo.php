...
class qiniuvideo
{
    ...
    var $image_size = 500;        //大小
    var $image_size_format = 'mb';        //图片大小单位
    var $image_type = ['flv','mp4'];        //图片类型
    //上传图片
    public function upload_image($file,$bucket)
    {
        ...
        if(!$this->checke_image_size($filePath)){
            return ["code"=>10010,"message"=>'文件过大！',"data"=>""];
        }
        if(!$this->checke_image_type($file)){
            return ["code"=>10011,"message"=>'类型有误！',"data"=>""];
        }
    ...
    public function checke_image_type($file){
        ...
        $file_type = \think\Validate::is($file, 'file');
        if(!$file_type){
            return false;
        }
        /*$img_data = getimagesize($file->getRealPath());
        $img_type = trim(strrchr(end($img_data), '/'),'/');*/
        $img_type = strtolower(pathinfo($file->getInfo('name'), PATHINFO_EXTENSION));
